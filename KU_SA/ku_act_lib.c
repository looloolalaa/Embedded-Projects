...
int ku_act_active() {
	...
	int ret = ioctl(fd, KU_ACT_ACTIVE, 0);
	close(fd);

	if(ret != -1)
		 printf("success to activate actuator\n");
	else
		printf("fail to activate actuator\n");
...
int ku_act_stop() {
	...
	int fd = open("/dev/ku_sa_dev", O_RDWR);
	int ret = ioctl(fd, KU_ACT_STOP, 0);
	close(fd);

	if(ret != -1)
		 printf("success to stop actuator\n");
	else
		printf("fail to stop actuator\n");
